...
void MaterialPropertiesTable::AddPropertiesToSurface(G4OpticalSurface* optSurface)
{
  G4cout<<fName<<" ---G4MaterialPropertiesTable---"<<G4endl;
  G4MaterialPropertiesTable::DumpTable();
...
void MaterialPropertiesTable::AddProperty(const G4String& key, 
...

{
  ...
  { 
    ...
    //std::sort(photonVector.begin(), photonVector.end(), std::greater<double>{});
  ...
  G4MaterialPropertiesTable::AddProperty(key, photonEnergies, propertyValues, createNewKey, spline);
...

{
  ...
  std::vector<G4double> propertyValues; 
  propertyValues.insert(propertyValues.end(), photonVector.size(), propertyValue*vUnit);   
  
  //Pass the filled vectors to the method of G4MaterialPropertiesTable. 
  G4MaterialPropertiesTable::AddProperty(key, photonVector, propertyValues, createNewKey, spline);
...
void MaterialPropertiesTable::CheckUnitsAndConvertToDimensionedDouble(const G4String& photonUnit, 
                                                                       const G4String& valueUnit, 
                                                                       G4double& pUnit, 
                                                                       G4double& vUnit
                                                                      )
{
  ...
  if ( !G4UnitDefinition::IsUnitDefined(photonUnit) )
  {
    ...
    G4Exception("MaterialPropertiesTable::CheckUnitsAndConvertToDimensionedDouble()", 
                "matPropError0", FatalException, ed
               );
  }
  ...
  {    
    ...
    {
      G4ExceptionDescription ed;
      ed << "Undefined energy or length unit: <" <<photonUnit<< ">. Command failed.";
      G4Exception("MaterialPropertiesTable::CheckUnitsAndConvertToDimensionedDouble()", 
                  "matPropError1", FatalException, ed
                 );
  ...
  {
    G4ExceptionDescription ed;
    ed << "Undefined property value unit: <" <<valueUnit<< ">. Command failed.";
    G4Exception("MaterialPropertiesTable::CheckUnitsAndConvertToDimensionedDouble()", 
