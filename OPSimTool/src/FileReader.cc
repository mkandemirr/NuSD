...
std::vector<G4double> FileReader::ReadSingleColumnIntoAVector(const G4String &filePath) 
{
  ...
  else 
  {
    ...
    G4Exception("FileReader::ReadSingleColumnIntoAVector()", "FileReaderErr1", FatalException, ed);  
  } 
...
std::vector< std::pair<double,double> > FileReader::ReadTwoColumnsIntoAVectorOfPairs(const G4String &filePath) 
{
  G4String line;
  ...
  { 
    G4ExceptionDescription ed;
    ed << "Unable to open file: "<<filePath;
    G4Exception("FileReader::ReadTwoColumnsIntoAVectorOfPairs()", "FileReaderErr2", FatalException, ed);  
  } 
